...
char *WebSocket_getdata(networkHandles *net, size_t bytes, size_t* actual_len)
{
	...
	if ( net->websocket )
	{
		...
		if ( !frame )
		{
			...
			if ( rc_t == TCPSOCKET_COMPLETE && in_frames && in_frames->first)
				frame = in_frames->first->content;
		}
		...
		if ( frame )
		{
			...
			*actual_len = frame->len - frame->pos; /* use the rest of the frame */
			...
			while (*actual_len < bytes) {
				const int rc_t = WebSocket_receiveFrame(net, actual_len);

				if (rc_t != TCPSOCKET_COMPLETE) {
					goto exit;
				}
		...
		do
		{
			...
			while ( is_final == 0 )
			{
				...
				if ( has_mask )
				{
					...
					memcpy( &mask_t[0], b, sizeof(uint32_t));
				}
...
int WebSocket_upgrade( networkHandles *net )
{
	...
	if ( net->websocket_key )
	{
		...
		MQTT_SHA1_Init( &ctx );
		MQTT_SHA1_Update( &ctx, ws_key, strlen(ws_key));
		MQTT_SHA1_Final( sha_hash, &ctx );
