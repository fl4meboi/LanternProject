...
int MQTT_SHA1_Update(SHA_CTX *c, const void *data, size_t len)
{
	int rv = 1;
...
int MQTT_SHA1_Final(unsigned char *md, SHA_CTX *c)
{
	int rv = 0;
...
int MQTT_SHA1_Final(unsigned char *md, SHA_CTX *ctx)
{
	...
	MQTT_SHA1_Update(ctx, pad, pad_amount);
...
int main(int argc, char *argv[])
{
	...
	while ( test_data[i].in != NULL )
	{
		...
		SHA_CTX c;
		int j;
		r[0] = MQTT_SHA1_Init( &c );
		r[1] = MQTT_SHA1_Update( &c, test_data[i].in, strlen(test_data[i].in));
		r[2] = MQTT_SHA1_Final( sha_out, &c );
