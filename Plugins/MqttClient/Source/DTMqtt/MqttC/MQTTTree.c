...
/*int TreeMaxDepth(MQTT_Tree *aTree);*/
void TreeRotate(MQTT_Tree* aTree, Node* curnode, int direction, int index);
Node* TreeBAASub(MQTT_Tree* aTree, Node* curnode, int which, int index);
void TreeBalanceAfterAdd(MQTT_Tree* aTree, Node* curnode, int index);
void* TreeAddByIndex(MQTT_Tree* aTree, void* content, size_t size, int index);
Node* TreeFindIndex1(MQTT_Tree* aTree, void* key, int index, int value);
Node* TreeFindContentIndex(MQTT_Tree* aTree, void* key, int index);
Node* TreeMinimum(Node* curnode);
Node* TreeSuccessor(Node* curnode);
Node* TreeNextElementIndex(MQTT_Tree* aTree, Node* curnode, int index);
Node* TreeBARSub(MQTT_Tree* aTree, Node* curnode, int which, int index);
void TreeBalanceAfterRemove(MQTT_Tree* aTree, Node* curnode, int index);
void* TreeRemoveIndex(MQTT_Tree* aTree, void* content, int index);


void MQTTTreeInitializeNoMalloc(MQTT_Tree* aTree, int(*compare)(void*, void*, int))
{
	memset(aTree, '\0', sizeof(MQTT_Tree));
...
MQTT_Tree* MQTTTreeInitialize(int(*compare)(void*, void*, int))
{
	...
	MQTT_Tree* newt = malloc(sizeof(MQTT_Tree));
#else
	MQTT_Tree* newt = mymalloc(__FILE__, __LINE__, sizeof(MQTT_Tree));
#endif
	if (newt)
		MQTTTreeInitializeNoMalloc(newt, compare);
	return newt;
}
...
{
	return MQTTTreeFindIndex(aTree, key, 0);
}
 ...
 */
void* MQTTTreeRemoveNodeIndex(MQTT_Tree* aTree, Node* curnode, int index)
{
...
void* TreeRemoveIndex(MQTT_Tree* aTree, void* content, int index)
{
	...
	return MQTTTreeRemoveNodeIndex(aTree, curnode, index);
}
...
{
	Node* curnode = MQTTTreeFindIndex(aTree, key, index);
	void* content = NULL;
	int i;
	...
	content = MQTTTreeRemoveNodeIndex(aTree, curnode, index);
	for (i = 0; i < aTree->indexes; ++i)
	{
		if (i != index)
...
{
	return MQTTTreeRemoveKeyIndex(aTree, key, 0);
}


int MQTTTreeIntCompare(void* a, void* b, int content)
{
	int i = *((int*)a);
	int j = *((int*)b);
...
int MQTTTreePtrCompare(void* a, void* b, int content)
{
	return (a > b) ? -1 : (a == b) ? 0 : 1;
}


int MQTTTreeStringCompare(void* a, void* b, int content)
{
...
int check(MQTT_Tree *t)
{
	Node* curnode = NULL;
	int rc = 0;

	curnode = MQTTTreeNextElement(t, curnode);
	while (curnode)
	{
...
int traverse(MQTT_Tree *t, int lookfor)
{
	Node* curnode = NULL;
	int rc = 0;
...
int test(int limit)
{
	...
	printf("MQTT_Tree initialized\n");

	srand(time(NULL));

	ip = malloc(sizeof(int));
	*ip = 2;
	MQTTTreeAdd(t, (void*)ip, sizeof(int));

	check(t);

	i = 2;
	void* result = MQTTTreeRemove(t, (void*)&i);
	if (result)
		free(result);
	...
	for (i = 0; i < limit; i++)
	{
		...
		ip = malloc(sizeof(int));
		*ip = rand();
		replaced = MQTTTreeAdd(t, (void*)ip, sizeof(int));
		...
		if (i==5)
			todelete = ip;
		printf("MQTT_Tree element added %d\n",  *ip);
		if (1 % 1000 == 0)
		{
			rc = check(t);
	...
	for (i = 0; i < limit; i++)
	{
		...
		Node* found = MQTTTreeFind(t, (void*)&parm);
		if (found)
			printf("MQTT_Tree find %d %d\n", parm, *(int*)(found->content));
		else
		{
			printf("%d not found\n", parm);
			traverse(t, parm);
	...
	for (i = limit -1; i >= 0; i--)
	{
		...
		found = MQTTTreeRemove(t, (void*)&parm);
		if (found)
		{
			printf("%d MQTT_Tree remove %d %d\n", i, parm, *(int*)(found));
			free(found);
		}
		else
		{
