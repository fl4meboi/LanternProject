		...
		int persistence_type, void* persistence_context,  MQTTAsync_createOptions* options)
{
	...
	if (!MQTT_UTF8_validateString(clientId))
	{
...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	if ((options->username && !MQTT_UTF8_validateString(options->username)) ||
		(options->password && !MQTT_UTF8_validateString(options->password)))
	{
		rc = MQTTASYNC_BAD_UTF8_STRING;
		goto exit;
	}
...
int MQTTAsync_subscribeMany(MQTTAsync handle, int count, char* const* topic, const int* qos, MQTTAsync_responseOptions* response)
{
	...
	{
		if (!MQTT_UTF8_validateString(topic[i]))
		{
			rc = MQTTASYNC_BAD_UTF8_STRING;
			break;
		}
							 ...
							 int qos, int retained, MQTTAsync_responseOptions* response)
{
	...
	if (!MQTT_UTF8_validateString(destinationName))
		rc = MQTTASYNC_BAD_UTF8_STRING;
	else if (qos < 0 || qos > 2)
		rc = MQTTASYNC_BAD_QOS;
...
int MQTTAsync_isComplete(MQTTAsync handle, MQTTAsync_token dt)
{
	...
	if (m->c && m->c->outboundMsgs->count > 0)
	{
		...
		{
			Messages* m_t = (Messages*)(current->content);
			if (m_t->msgid == dt)
				goto exit;
		}
	}
