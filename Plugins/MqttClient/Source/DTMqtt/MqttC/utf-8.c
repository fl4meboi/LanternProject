...
int MQTT_UTF8_validateString(const char* string)
{
	...
	if (string != NULL)
	{
		rc = MQTT_UTF8_validate((int)strlen(string), string);
	}
	FUNC_EXIT_RC(rc);
	return rc;
}
...
int main (int argc, char *argv[])
{
	...
	{
		if (!MQTT_UTF8_validate(valid_strings[i].len, valid_strings[i].data))
		{
	...
	{
		if (MQTT_UTF8_validate(invalid_strings[i].len, invalid_strings[i].data))
		{
	...
	MQTT_UTF8_validateString(NULL);
	MQTT_UTF8_validate(1, NULL);
